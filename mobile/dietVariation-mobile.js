...
(function() {
    ...
    function renderSummaryBox(analysis) {
        const overused = (analysis.overusedFoods || []).slice(0, 3).map(item =>
            `<div class="variation-summary-item variation-warn">${esc(item.name)} · ${item.count}/7</div>`
        ).join('');

        const missing = (analysis.missingCategories || []).slice(0, 3).map(item =>
        ...
        return `
            <div class="variation-panel">
                ...
                <div class="variation-subtitle">Η λογική αλλαγής της επόμενης εβδομάδας</div>
    ...
    window.PegasusDietVariationMobile = {
        ...
        resetPrefs: function() {
            try { localStorage.removeItem('pegasus_diet_variation_prefs_v1'); } catch (_) {}
            this.render();
        }
    };
