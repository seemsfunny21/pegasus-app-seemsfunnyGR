...
(function () {
    ...
    const FILE_MAP = {
        ...
        'desktopBoot.js': { category: 'Desktop Boot', role: 'Το βασικό desktop boot sequence, μαζί με weather UI, calendar/backup wiring, desktop sync modal bridge και status label.' },
        ...
        'weightTracker.js': { category: 'Tracking', role: 'Weight tracking σώματος, partner mode helpers, exercise weight state και related storage normalization.' },
        ...
        'reporting.js': { category: 'Reporting', role: 'Morning report / manual report sending, previous-day finalizer και fallback builder από food/cardio/workout logs όταν λείπει pending report.' },
        ...
        'desktopActions.js': { category: 'Desktop UI', role: 'Desktop button actions, rendering/video updates και tracking flows που συνδέουν UI με engine behaviors.' },
        'desktopBoot.js': { category: 'Desktop Boot', role: 'Το βασικό desktop boot sequence: navbar, initial render, weather sync, backup/calendar wiring, sync modal/status και onload coordination.' },
        'calorieRuntime.js': { category: 'Nutrition', role: 'Ημερήσιο kcal/protein runtime, labels, targets και calorie UI ενημερώσεις.' },
        'runtimeBridge.js': { category: 'App Coordination', role: 'Bridge ανάμεσα σε legacy runtime/UI και στο νέο engine state. Κρατά snapshots, audio runtime/beep fallback και control state sync.' },
        'selfCheckRunner.js': { category: 'Diagnostics', role: 'Regression/self-check runner, module integrity checks και γρήγορο PASS/WARN health check του build.' },
        'debug.js': { category: 'Diagnostics', role: 'Debug helpers, runtime warnings, summaries, controls/uiState outputs και extra console utilities.' },
        ...
        'mobile/mobileApp.js': { category: 'Mobile Runtime', role: 'Κεντρικό mobile runtime: view routing, bottom boundary, mobile UI rendering, service worker registration και bridge view rendering.' }
    };
