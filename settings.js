...
/* =============================================================
   PEGASUS SETTINGS ENGINE - MODULAR BUILD v5.0
   Protocol: Strict Data Analyst - Global Sync & Car Logic
   ============================================================= */

window.PegasusSettings = {
    init: function() {
        console.log("⚙️ PEGASUS: Settings Module Initializing...");

        // 1. Σύνδεση Κεντρικού Κουμπιού (Navbar)
        const btnUI = document.getElementById("btnSettingsUI");
        if (btnUI) {
            btnUI.onclick = () => this.togglePanel();
        }

        // 2. Σύνδεση Κουμπιού Αποθήκευσης (Settings Panel)
        const saveBtn = document.getElementById("btnSaveSettings");
        if (saveBtn) {
            saveBtn.onclick = () => this.saveAll();
        }

        // 3. Αρχική Φόρτωση Τιμών στα Inputs
        this.loadToInputs();
    },
    ...
    togglePanel: function() {
        ...
        if (panel) {
            ...
            if (!isVisible) this.loadToInputs();
        }
    },

    loadToInputs: function() {
        // Σωματομετρικά & Στόχοι
        const fields = {
            'userWeightInput': 'pegasus_weight',
            'userHeightInput': 'pegasus_height',
            'userAgeInput': 'pegasus_age',
            'goalKcalInput': 'pegasus_goal_kcal',
            'goalProteinInput': 'pegasus_goal_protein'
        };
        ...
        // Φόρτωση Weekly Targets (Στήθος, Πλάτη, κλπ.)
        const targets = JSON.parse(localStorage.getItem("pegasus_muscle_targets") || "{}");
        ["Στήθος", "Πλάτη", "Πόδια", "Χέρια", "Ώμοι", "Κορμός"].forEach(m => {
            const el = document.getElementById(`target${m}Input`);
            if (el) el.value = targets[m] || "";
        });
    },

    saveAll: async function() {
        console.log("💾 PEGASUS: Committing changes to Local & Cloud...");

        // 1. Αποθήκευση Βασικών Ρυθμίσεων
        ...
        const newTargets = {};
        ["Στήθος", "Πλάτη", "Πόδια", "Χέρια", "Ώμοι", "Κορμός"].forEach(m => {
            const val = document.getElementById(`target${m}Input`).value;
            if (val) newTargets[m] = parseInt(val);
        });
        localStorage.setItem("pegasus_muscle_targets", JSON.stringify(newTargets));

        // 3. MASTER CLOUD PUSH
        if (window.PegasusCloud && window.PegasusCloud.isUnlocked) {
            try {
                // Ενημερώνουμε το timestamp για να ξέρουν οι άλλες συσκευές ότι άλλαξαν οι ρυθμίσεις
                localStorage.setItem("pegasus_last_push", Date.now().toString());
                await window.PegasusCloud.push(true);
                alert("✅ Ρυθμίσεις αποθηκεύτηκαν και συγχρονίστηκαν!");
            } catch (e) {
                alert("⚠️ Αποθήκευση τοπικά (Σφάλμα συγχρονισμού).");
            }
        } else {
            alert("✅ Ρυθμίσεις αποθηκεύτηκαν τοπικά.");
        }

        // Κλείσιμο Panel & Refresh UI
...
// Εκκίνηση
window.addEventListener("load", () => window.PegasusSettings.init());
